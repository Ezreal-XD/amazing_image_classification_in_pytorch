...
def train_model(args):
    ...
    global learnable_gt
    learnable_gt = torch.eye(args.classes).cuda()
    # learnable_gt.requires_grad = True
    # criteria_gt = nn.L1Loss().cuda()
    # optimizer_gt = torch.optim.SGD([learnable_gt], lr=args.lr, momentum=0.9, weight_decay=1e-4)
...
def train(args, train_loader, model, criterion, optimizer, epoch):
    ...
    for iteration, batch in enumerate(train_loader, 0):
        ...
        # current gt
        gt = torch.empty((args.batch_size, args.classes)).cuda()
        for idx, v in enumerate(labels):
            gt[idx] = learnable_gt[v]
        ...
        for idx, v in enumerate(labels):
            learnable_gt[v] -= 0.01 * (learnable_gt[v] - output[idx])
